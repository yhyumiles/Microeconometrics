...
\begin{frame}{Algorithm for calculating $\hat{\mathbf{L}}$}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Define $\text{shrink}_{\lambda}(\mathbf{A})=\mathbf{S\tilde{\Sigma}R^\mathsf{T}}$ and start with the initial choice $\mathbf{L_1}(\lambda, \mathcal{O})=\mathbf{P}_\mathcal{O} (\mathbf{Y})$ (The missing starts $0$.)
            \[
...
\begin{frame}{Summary and Conclusion}
    \begin{itemize}
        ...
        \item The critical differnce with previus estimators is that MC-NNM \alert{holistically restricts} the factors by regularization woth sparsity
